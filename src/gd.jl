...
function optimize(
    ...
    if _hasconverged
        verbosity >= 2 &&
            @info @sprintf(
            "GD: converged after %d iterations and time %s: f = %.12e, ‖∇f‖ = %.4e",
            numiter, format_time(t), f, normgrad
        )
    else
        verbosity >= 1 &&
            @warn @sprintf(
            "GD: not converged to requested tol after %d iterations and time %s: f = %.12e, ‖∇f‖ = %.4e",
            numiter, format_time(t), f, normgrad
        )
    end
