...
"""
function (ls::HagerZhangLineSearch)(
        fg, x₀, η₀, fg₀ = fg(x₀);
        retract = _retract, inner = _inner,
    ...
    iter = HagerZhangLineSearchIterator(
        fg, retract, inner, p₀, η₀, initialguess,
    ...
    while true
        ...
        verbosity >= 3 &&
            @info @sprintf(
            "Linesearch iteration step %d, function evaluation count %d:\n[a,b] = [%.2e, %.2e], dϕᵃ = %.2e, dϕᵇ = %.2e, ϕᵃ - ϕ₀ = %.2e, ϕᵇ - ϕ₀ = %.2e",
            k, numfg, a.α, b.α, a.dϕ, b.dϕ, a.ϕ - ϕ₀, b.ϕ - ϕ₀
        )
        if done
            verbosity >= 2 &&
                @info @sprintf(
                "Linesearch converged after %d iterations and %d function evaluations:\nα = %.2e, dϕ = %.2e, ϕ - ϕ₀ = %.2e",
                k, numfg, α, dϕ, f - ϕ₀
            )
        ...
        elseif k >= maxiter || numfg >= maxfg
            verbosity >= 1 &&
                @warn @sprintf(
                "Linesearch not converged after %d iterations and %d function evaluations:\nα = %.2e, dϕ = %.2e, ϕ - ϕ₀ = %.2e",
                k, numfg, α, dϕ, f - ϕ₀
            )
            return x, f, g, ξ, α, numfg
        else
...
function Base.iterate(iter::HagerZhangLineSearchIterator)
    ...
    if iter.acceptfirst
        ...
        verbosity >= 4 &&
            @info @sprintf(
            "  Linesearch initial step: c = %.2e, dϕᶜ = %.2e, ϕᶜ - ϕ₀ = %.2e, exact wolfe = %d, approx wolfe = %d",
        ...
        verbosity >= 4 &&
            @info @sprintf(
            "  Linesearch initial step (cannot be accepted): c = %.2e, dϕᶜ = %.2e, ϕᶜ - ϕ₀ = %.2e",
            c.α, c.dϕ, c.ϕ - p₀.ϕ
        )
    end
    ...
    verbosity >= 4 &&
        @info @sprintf(
        "  Linesearch initial bracket: [a,b] = [%.2e, %.2e], dϕᵃ = %.2e, dϕᵇ = %.2e, ϕᵃ - ϕ₀ = %.2e, ϕᵇ - ϕ₀ = %.2e",
    ...
    elseif (b.α - a.α) < eps(one(a.α))
        verbosity >= 1 &&
            @warn @sprintf(
            "  Linesearch bracket converged to a point without satisfying Wolfe conditions: [a,b] = [%.2e, %.2e], dϕᵃ = %.2e, dϕᵇ = %.2e, ϕᵃ - ϕ₀ = %.2e, ϕᵇ - ϕ₀ = %.2e",
            a.α, b.α, a.dϕ, b.dϕ, a.ϕ - p₀.ϕ, b.ϕ - p₀.ϕ
        )
        return (a.x, a.f, a.∇f, a.ξ, a.α, a.dϕ), (a, b, numfg, true)
    else
...
function Base.iterate(
    ...
    verbosity >= 4 &&
        @info @sprintf(
        "  Linesearch updated bracket (secant2): [a,b] = [%.2e, %.2e], dϕᵃ = %.2e, dϕᵇ = %.2e, ϕᵃ - ϕ₀ = %.2e, ϕᵇ - ϕ₀ = %.2e",
        a.α, b.α, a.dϕ, b.dϕ, a.ϕ - p₀.ϕ, b.ϕ - p₀.ϕ
    )
    if a.α == b.α
        return (a.x, a.f, a.∇f, a.ξ, a.α, a.dϕ), (a, b, numfg, true)
    ...
    if b.α - a.α > iter.parameters.γ * dα
        ...
        verbosity >= 4 &&
            @info @sprintf(
            "  Linesearch updated bracket (bisection): [a,b] = [%.2e, %.2e], dϕᵃ = %.2e, dϕᵇ = %.2e, ϕᵃ - ϕ₀ = %.2e, ϕᵇ - ϕ₀ = %.2e",
            a.α, b.α, a.dϕ, b.dϕ, a.ϕ - p₀.ϕ, b.ϕ - p₀.ϕ
        )
    end
    if a.α == b.α
...
function update(
        iter::HagerZhangLineSearchIterator, a::LineSearchPoint, b::LineSearchPoint,
        αc
    )
    p₀ = iter.p₀
    c₁ = iter.parameters.c₁
    ...
    verbosity >= 4 &&
        @info @sprintf(
        "  Linesearch update step (U1-U2): c = %.2e, dϕᶜ = %.2e, ϕᶜ - ϕ₀ = %.2e, exact wolfe = %d, approx wolfe = %d",
...
function bisect(iter::HagerZhangLineSearchIterator, a::LineSearchPoint, b::LineSearchPoint)
    ...
    while true
        if (b.α - a.α) <= eps(one(a.α))^(3 // 4) || numfg >= iter.parameters.maxfg
            if verbosity >= 1
                @warn @sprintf(
                    "  Linesearch bisection failure: [a, b] = [%.2e, %.2e], b-a = %.2e, dϕᵃ = %.2e, dϕᵇ = %.2e, (ϕᵇ - ϕᵃ)/(b-a) = %.2e",
        ...
        verbosity >= 4 &&
            @info @sprintf(
            "  Linesearch bisection update (U3): d = %.2e, dϕᵈ = %.2e, ϕᵈ - ϕ₀ = %.2e, exact wolfe = %d, approx wolfe = %d",
...
function bracket(iter::HagerZhangLineSearchIterator{T}, c::LineSearchPoint) where {T}
    ...
    while true
        while !(isfinite(c.ϕ) && isfinite(c.dϕ)) && numfg < iter.parameters.maxfg
            ...
            verbosity >= 4 &&
                @info @sprintf(
                "  Linesearch bracket step: c = %.2e, dϕᶜ = %.2e, ϕᶜ - ϕ₀ = %.2e",
                c.α, c.dϕ, c.ϕ - p₀.ϕ
            )
        end
